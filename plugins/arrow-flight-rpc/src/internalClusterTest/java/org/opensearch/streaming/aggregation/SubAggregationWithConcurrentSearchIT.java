...
public class SubAggregationWithConcurrentSearchIT extends OpenSearchIntegTestCase {
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream1", "metrics.size", 1));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream2", "metrics.size", 2));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream3", "metrics.size", 3));
        }
        ...
        for (int i = 0; i < 10; i++) {
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream1", "metrics.size", 11));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream2", "metrics.size", 12));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream3", "metrics.size", 13));
        }
        ...
        bulkRequest = new BulkRequest();
        for (int i = 0; i < 10; i++) {
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream1", "metrics.size", 21));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream2", "metrics.size", 22));
            bulkRequest.add(new IndexRequest(INDEX_NAME).source(XContentType.JSON, "aws.cloudwatch.log_stream", "stream3", "metrics.size", 23));
        }
        bulkResponse = client().bulk(bulkRequest).actionGet();
        assertFalse(bulkResponse.hasFailures());
        client().admin().indices().flush(new FlushRequest(INDEX_NAME).force(true)).actionGet();
        ...
        segmentResponse.getIndices().get(INDEX_NAME).getShards().values().forEach(indexShardSegments -> {
            assertTrue("Expected at least " + MIN_SEGMENTS_PER_SHARD + " segments but found " +
