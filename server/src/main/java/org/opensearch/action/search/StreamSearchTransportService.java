...
public class StreamSearchTransportService extends SearchTransportService {
    ...
    public static void registerStreamRequestHandler(StreamTransportService transportService, SearchService searchService) {
        transportService.registerRequestHandler(
            ...
            (request, channel, task) -> {
                searchService.executeQueryPhaseStream(
                    ...
                    new StreamChannelListener<>(channel, QUERY_ACTION_NAME, request),
        ...
        transportService.registerRequestHandler(
            ...
            (request, channel, task) -> {
                searchService.executeFetchPhase(
                    request,
                    (SearchShardTask) task,
                    new StreamChannelListener<>(channel, FETCH_ID_ACTION_NAME, request),
                    ThreadPool.Names.STREAM_SEARCH
                );
            }
        );
        transportService.registerRequestHandler(
            ...
            ShardSearchRequest::new,
            (request, channel, task) -> {
                searchService.canMatch(request, new StreamChannelListener<>(channel, QUERY_CAN_MATCH_NAME, request));
            }
        );
        transportService.registerRequestHandler(
        ...
        transportService.registerRequestHandler(
            ...
            (request, channel, task) -> searchService.executeDfsPhase(
                ...
                false,
                (SearchShardTask) task,
                new StreamChannelListener<>(channel, DFS_ACTION_NAME, request),
                ThreadPool.Names.STREAM_SEARCH
            )
        );
    }
    ...
    public void sendExecuteQuery(
        ...
        final StreamingSearchActionListener streamListener = (StreamingSearchActionListener) listener;
