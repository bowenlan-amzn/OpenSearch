...
public class ContextIndexSearcher extends IndexSearcher implements Releasable {
    ...
    protected void search(LeafReaderContextPartition[] partitions, Weight weight, Collector collector) throws IOException {
        logger.debug("searching for {} partitions of shard [{}]",  partitions.length, searchContext.shardTarget().getShardId().id());
    ...
    protected void searchLeaf(LeafReaderContext ctx, int minDocId, int maxDocId, Weight weight, Collector collector) throws IOException {
        ...
        if (searchContext.isStreamSearch()) {
            logger.debug("Stream intermediate aggregation for segment [{}], shard [{}]", ctx.ord, searchContext.shardTarget().getShardId().id());
